...
; ( function () {
  ...
  var Client = WinJS.Class.define(
     ...
     */
    {
      ...
      post: function ( method, data, url, accessToken, accessTokenSecret, extraParams ) {
        method = method || "POST";
        var paramStr = "";
        if ( extraParams ) {
          for ( var key in extraParams ) if ( extraParams.hasOwnProperty( key ) ) {
        ...
        var options = {
          ...
          headers: {
            "Accept": "*/*",
            "User-Agent": this.userAgent_,
            "Authorization": this.createParamsStr_( this.prepareParameters_( accessToken, accessTokenSecret, null, "POST", url, extraParams ) )
          },
        };
